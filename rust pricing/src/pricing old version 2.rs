...
impl PricingProblem {
    ...
    // Modified edge filtering in build_edges
    fn build_edges(&mut self, dual_values: &HashMap<String, f64>) {
        let max_edge_distance = 50.0; // km - adjust based on your problem
        
        // Precompute all locations
        let mut locations = Vec::new();
        locations.extend(self.warehouses.iter().map(|(id, wh)| (id.clone(), (wh.lat, wh.lng))));
        locations.extend(self.customers.iter().map(|(id, cust)| (id.clone(), (cust.lat, cust.lng))));
        
        // Build k-d tree or similar for spatial queries if needed
        
        for (i, (u, coords_u)) in locations.iter().enumerate() {
            for (j, (v, coords_v)) in locations.iter().enumerate() {
                if i == j {
                    continue;
                }
                
                let distance = self.haversine_distance(*coords_u, *coords_v);
                if distance > max_edge_distance {
                    continue;
                }
                
                // Additional geographic filtering
                if u.starts_with("C_") && v.starts_with("C_") {
                    // For customer-customer edges, enforce some direction consistency
                    let angle = self.calculate_angle(*coords_u, *coords_v);
                    if angle.abs() > 60.0 { // 60 degree maximum deviation
                        continue;
                    }
                }
                
                self.add_edge(u, v, dual_values);
            }
        }
    }

    // Helper function for geographic filtering
        ...
        for start_wh in self.warehouses.keys() {
            ...
            while let Some(current) = queue.pop() {
                for edge in self.graph.edges(current.last_node) {
                    ...
                    // Only allow returning to starting warehouse
                    ...
                    if next_node.starts_with("C_") {
                        if customer_count >= self.max_stops || current.path.contains(next_node) {
                            continue;
                        }
                    }
